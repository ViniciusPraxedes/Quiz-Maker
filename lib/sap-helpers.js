...
export async function fetchSapLessonContent(item, sap_cookie) {
  ...
  // Check if item is a full HTTP URL
  if (item.trim().startsWith("http")) {
    // Trim input item URL string
    let url = item.trim();
    // Extract slug identifier or default to fallback slug
    extracted_unit_id = url.split("/").pop() || "sap-lesson";
    // Set target fetch URL to direct page URL
    sap_api_url = url;
  ...
  let response = await fetch(sap_api_url, {
  ...
  // Check if API endpoint response returned error and item is a full HTTP URL
  if (!response.ok && !is_next_data && item.trim().startsWith("http")) {
    // Set target URL to full HTTP page URL
    sap_api_url = item.trim();
    // Enable Next.js HTML page data parser flag
    is_next_data = true;
    // Perform fallback fetch request on full page URL
    response = await fetch(sap_api_url, {
      // Specify HTTP GET method
      method: "GET",
      // Pass headers
      headers: headers,
